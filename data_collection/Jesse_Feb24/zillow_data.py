...
GET_INFO_XPATH_SOLD_PRICE = '''//div[@id="home-value-wrapper"]/div[1]/div[1]/span[2]/text()'''
...
# Load user agents
USER_AGENTS_FILE = 'C:\Users\Jesse\Desktop\user_agents.txt'
USER_AGENTS = []
...
def get_property_by_zpid(zpid):
    ...
    sold_price = None
    try:
        sold_price_raw = tree.xpath(GET_INFO_XPATH_SOLD_PRICE)
        if len(sold_price_raw) > 0:
            sold_price = float(sold_price_raw[0].replace(',', '').strip(' $'))
    except Exception:
        pass
